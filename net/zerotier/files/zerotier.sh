...
proto_zerotier_setup() {
	...
	local network_id secret port config_path copy_config_path delay
	local args=""

	proto_add_host_dependency "$interface" '' "wan"
	...
	config_get delay "${interface}" "delay" 0
	...
	[ -n "$delay" -a "$delay" -gt 0 ] && sleep "$delay"
